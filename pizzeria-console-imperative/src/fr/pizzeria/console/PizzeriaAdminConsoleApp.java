...
public class PizzeriaAdminConsoleApp {
	public static void main(String[] args) {
		...
		do {
			...
			case 3:
				...
				int choixMod = sc.nextInt();

				if (choixMod >= 0 && choixMod <= 99) {
					System.out.println("");
				}
	...
	private static void afficherPizza(Object[][] listePizza) {
		int i = 0;
		...
		while (i < listePizza.length) {
			if (listePizza[i] != null) {
				System.out
						.println(i + " " + listePizza[i][0] + " -> " + listePizza[i][1] + " " + listePizza[i][2] + "€");
			}
			i++;
		}
	}
